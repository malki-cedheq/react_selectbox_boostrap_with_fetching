...
export default class SelectBoxFetchPropsTipoReceita extends React.Component {
  ...
  componentDidMount() {
    fetch(`${baseURL}/tipos_receitas`)
      ...
      .then((response) => {
        let responseFormat = response.tipo_receita.map(
          ({ id_tipo_receita: id, descricao: name }) => ({
            id,
            name,
          })
        );
    ...
    return (
      <div className="container mt-4">
        ...
        <SelectBox
          ...
          /*onChange={this.onChange}*/ //para onChange tratado neste objeto
          onChange={this.props.onChange} //para onChange tratado pelo Parent
        />
      </div>
    );
  }
